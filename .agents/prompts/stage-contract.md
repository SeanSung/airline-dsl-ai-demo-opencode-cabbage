...
- **requirements**：渐进式澄清（Decision Map），一次只问一个问题；领域术语写入根 CONTEXT.md；**PRD 必须含可判定验收标准（每条带判定方法、无模糊词、覆盖主路径/边界/失败降级），并逐条同步为 Parent Issue body checklist；Outcome/业务结果指标不得充作验收**
